...
start_note(fsm_t * p_this, double freq, uint32_t duration){
    fsm_buzzer_t *p_fsm = (fsm_buzzer_t *)(p_this);
    p_fsm->p_melody-> p_durations = (uint16_t) (duration / p_fsm->player_speed);
    port_buzzer_set_note_frequency(p_fsm->buzzer_id, p_fsm->p_melody->p_notes);
    port_buzzer_set_note_duration(p_fsm->buzzer_id, p_fsm->p_melody->p_durations);
}
...
static void do_play_note(fsm_t * p_this){
    ...
    double current_duration = p_fsm->p_melody-> p_durations[note_index];
...
fsm_trans_t fsm_trans_buzzer[] = {
    ...
    { PAUSE_NOTE, check_resume, PLAY_NOTE, },
